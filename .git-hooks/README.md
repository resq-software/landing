# Git Hooks — ResQ Landing

This directory contains the project's git hooks. They enforce code quality, security, and workflow conventions for the ResQ Landing site (TypeScript/Bun).
...
| Hook | Purpose |
|------|---------|
| `pre-commit` | Large file guard (1 MB limit), secrets scan (gitleaks / grep fallback), Biome auto-format + re-stage changed `.ts`/`.tsx`/`.js`/`.jsx`/`.json`/`.css` files |
...
| `pre-push` | Force-push guard on `main`, branch naming convention, Biome lint check |
| `post-checkout` | Auto `bun install` when `bun.lockb` changes between branches; auto `pnpm install` when `pnpm-lock.yaml` changes |
| `post-merge` | Auto `bun install` / `pnpm install` when lock files change after a merge |
